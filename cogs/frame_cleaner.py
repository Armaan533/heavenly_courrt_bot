...
class FrameTestModal(discord.ui.Modal, title="Frame Rendering Matrix"):
    ...
    def process_frame(self, card_bytes, frame_bytes):
        ...
        with Image.open(BytesIO(card_bytes)).convert("RGBA") as card_img, \
            ...
            edge_keep = np.power(1.0 - centre, 0.55)
            ...
            noise_floor = 0.018 + 0.045 * centre
            phys_gate = _smoothstep(noise_floor, noise_floor + 0.12, physical_alpha)
            phys_alpha = physical_alpha * phys_gate * (0.55 + protect * 0.85 + sat_conf * 0.80)

            dark_alpha = dark_conf * protect

            raw_alpha = np.maximum.reduce([color_alpha, sat_alpha, phys_alpha, dark_alpha])

            detail = np.maximum.reduce([sat_conf, bright_conf, dark_conf * 0.35])
            clear_window = _smoothstep(0.62, 1.0, centre) * (1.0 - protect * 0.86)
            flat_bg = clear_window * (1.0 - detail)

            raw_alpha *= (1.0 - flat_bg * 0.92)
