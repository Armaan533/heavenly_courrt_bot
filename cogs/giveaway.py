...
class GiveawayEntryView(discord.ui.View):
    ...
    async def enter_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if msg_id not in data["active"]:
            return await interaction.response.send_message("❌ This giveaway is no longer active or the artifact has dispersed.", ephemeral=True)
...
class GiveawayCog(commands.Cog):
    ...
    async def wait_for_kci(self, channel, author, seconds, clan_bonus, booster_bonus):
        ...
        try:
            ...
            return await channel.send("❌ Setup timed out.")

        embed_data = karuta_msg.embeds[0]
        lines = [l.strip() for l in (embed_data.description or "").split("\n") if l.strip()]
        if not lines: return await channel.send("❌ Could not read card metadata.")
    ...
    async def start_wizard(self, interaction: discord.Interaction, clan_bonus: int = 0, booster_bonus: int = 0):
        ...
        await interaction.response.send_message(embed=discord.Embed(title="Giveaway Setup", description="Select the type:", color=0x8b0000), view=GiveawayTypeSelect(self, interaction.user, clan_bonus, booster_bonus), ephemeral=True)
    ...
    async def reroll_giveaway(self, interaction: discord.Interaction, message_id: str):
        ...
        if message_id not in data["ended"]:
            return await interaction.response.send_message("❌ History not found.", ephemeral=True)
